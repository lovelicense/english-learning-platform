...
최근 업데이트 (2026-05-09):
- 모바일 웹 패리티 Ticket P1~P12 완료
- 모바일 AI 대화 섹션, 다이얼로그 연습 전역 라이브러리, 패턴형 연습/저장, 최근 복습 기록 확장 반영
- 모바일 `TTS 완료 표현` 화면에 반복 횟수 / 한국어 포함 재생 / 학습 설정형 플레이리스트 추가
- 모바일 `Learning Assets` 화면 추가
  - 패턴/단어 진도 요약
  - 약한 유형 확인
  - 패턴/단어 자산 브라우저
  - 관련 표현 상세/연습 연결
- 모바일 `Learning Assets` 화면에 `데이터 Export` 추가
  - `한글표현 자산`, `영어표현 자산` JSON / CSV 저장
  - Android 폴더 선택 저장 / iOS 공유 시트 연결
- 모바일 `record` 화면에 `파일 가져오기 / 수동 업로드` 추가
  - `wav`, `m4a`, `mp3`, `mp4`, `aac` 지원
  - 최대 `50MB` 검증
  - 가져온 파일도 recording session 업로드 / finalize / process 흐름에 연결
- 모바일 `record` 화면에 실패 복구 UI 추가
  - 실패 파트 / 실패 작업 / 세션 오류 메시지 표시
  - `실패 파트 다시 업로드`, `처리 다시 요청` 액션 추가
  - 파트별 size / duration / updated 시각 표시
- 모바일 `expression/[id]` 화면에 표현 삭제 추가
  - 삭제 확인 다이얼로그
  - 삭제 후 표현 목록으로 복귀
  - 오늘의 복습 제외 안내 반영
- 모바일 `Quick Save`는 하단 탭을 유지하는 구조로 조정
  - `Home` 카드로 진입해도 탭 바 유지
  - 별도 탭 버튼 추가 없이 hidden tab route로 처리
- 패턴형 문제 채점이 웹/모바일 공통으로 `현재 패턴형 문제 정답` 기준으로 이뤄지도록 백엔드 채점 컨텍스트 수정
...
- [모바일 웹 기능 패리티 티켓](./mobile_web_parity_tickets.md)
...
✔ 말하기 테스트(텍스트/음성)
✔ 오늘의 복습
✔ 모바일 AI 대화 홈 / 세션 / 설정 / 후속 학습 연결
✔ 다이얼로그 연습 전역 라이브러리 / 제목 수정
✔ 패턴형 연습 / 패턴형 결과 저장
✔ 최근 복습 기록 재진입 액션
✔ TTS 라이브러리 고급 재생 옵션
✔ 모바일 학습 자산 진도 / 패턴 DB / 단어 DB 화면
✔ 모바일 학습 자산 JSON / CSV export
✔ 모바일 파일 가져오기 / 수동 업로드
✔ 모바일 업로드 실패 파트 재시도 / 고급 상태 화면
✔ 모바일 표현 삭제 / 세부 관리 액션
